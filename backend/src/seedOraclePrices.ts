/**
 * Push initial oracle prices to all 4 dark pool markets (v3/v2 redeployment).
 * Run after deployment to seed initial prices.
 *
...
async function seedOraclePrices() {
  console.log('=== Pushing Initial Oracle Prices (v3/v2 contracts) ===\n');

  // Prices in u64 with 6-decimal precision, scaled to fit MAX_PRICE (100_000_000u64 = $100)
  // ALEO ~$0.05 → 45960u64 (no scale, fits directly)
  ...
  const pricePushes = [
    { program: config.darkpoolProgramId, symbol: 'ALEO', price: '45960u64',     round: '1u64' },
  ...
  for (let i = 0; i < pricePushes.length; i++) {
    ...
    console.log(`[${i + 1}/${pricePushes.length}] update_oracle_price on ${p.program} → ${p.price} (${p.symbol})`);
  ...
  console.log(`\n=== Done! All ${pricePushes.length} oracle prices seeded. ===`);
}
